...
# charge la page et les boxes directement dans le dictionnaire principal sans utiliser de return
def submit(self, info):
    ...
    trimmed_fens = list()
    trimmed_legends = list()
    trimmed_symbols = list()
    j = 0
    while j < info["diags_value"]:
        trimmed_fens.append(info["fens"][j])
        trimmed_legends.append(info["legends"][j])
        trimmed_symbols.append(info["symbols"][j])
        j += 1
# formats the fens (flips if needed)
    flip = info["flip_state"]
    for fen, sym, arr  in zip(trimmed_fens, trimmed_symbols, info["arrows"]):
...
# draws the boards
    ...
    i_state = info["numDiag_state"]
    index = info["numDiag_value"]
    c_state = info["color_state"]
    l_state = info["legend_state"]
    legends = trimmed_legends
...
# draws the boxes (board + decorations)
    ...
    orient = info["format_text"]
    t_state = info["title_state"]
    title = info["title_text"]
    n_state = info["numPage_state"]
    num = info["numPage_value"]
    col = info["cols_value"]
    margin = info["margin_value"]
        ...
        if fen == "":
            ...
            if i < 2:
                ...
                for row in rows:
                    ...
                    if squares != 8:
                        return("Nombre de cases invalide")
                if i != 8:
                    return("Nombre de rangées invalide")
            return("OK")
